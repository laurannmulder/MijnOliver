...
export function Logo({ className = '', ondertitel = true }: { className?: string; ondertitel?: boolean }) {
  return (
    <Link href={`${BASIS}`} className={`group flex items-center gap-3 ${className}`} aria-label="United Visions, naar de homepage">
      ...
      <span className="leading-none">
        ...
        {ondertitel && <span className="label mt-1 block text-[0.6rem] text-zacht">Live event solutions</span>}
      </span>
    </Link>
  )
}
